...
test('environment fingerprint includes the runner and toolchain contract', () => {
  ...
  assert.notEqual(environmentFingerprint(base), environmentFingerprint(differentBunSource));
  assert.notEqual(environmentFingerprint(base), environmentFingerprint(differentBunSha256));
...
test('CI workflow routes build pools through the pinned environment action', () => {
  ...
  assert.match(environmentAction, /ln -s "\$bun_path" "\$bun_shim_dir\/bunx"/);
  assert.match(environmentAction, /shim_bunx_version=/);
  ...
  assert.match(environmentAction, /--retry 8 --retry-delay 2 --retry-max-time 180 --retry-all-errors/);
  assert.match(environmentAction, /bun-linux-x64\.zip/);
  assert.match(environmentAction, /install -m 0755 "\$bun_binary" "\$BUN_INSTALL\/bin\/bun"/);
  assert.doesNotMatch(environmentAction, /uses:\s+oven-sh\/setup-bun@v2/);
  ...
  assert.match(environmentAction, /trusted_bun_path="\$trusted_home\/\.bun\/bin\/bun"/);
  assert.match(environmentAction, /CI_TRUSTED_BUN_PATH=\$trusted_bun_path/);
  assert.match(environmentAction, /CI_TRUSTED_WASM_PACK_PATH=\$trusted_wasm_pack_path/);
  assert.doesNotMatch(environmentAction, /run: node scripts\/ci\/bun-runtime-admission\.mjs/);
  assert.match(environmentAction, /cp -L -- "\$trusted_bun_path" "\$staged_bun_path"/);
  assert.match(environmentAction, /sha256sum "\$staged_bun_path"/);
  assert.match(environmentAction, /bun-runtime-admission\.mjs/);
  assert.match(environmentAction, /if: \$\{\{ env\.CI_BUN_ADMISSION_MODE == 'download' \}\}/);
  assert.match(environmentAction, /CI_BUN_PATH="\$bun_path" "\$bun_path" scripts\/ci\/bun-runtime-admission\.mjs --verify/);
  assert.match(environmentAction, /run: '\"\$CI_BUN_PATH\" scripts\/ci\/editor-ci-environment\.mjs'/);
