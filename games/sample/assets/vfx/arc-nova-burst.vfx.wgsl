...
fn vfx_spawn(ctx: VfxSpawnContext, particle: ptr<function, VfxParticle>) {
  ...
  let speed = 3.2 + vfx_random_spawn(ctx, 1u) * 1.8;
  let lift = (vfx_random_spawn(ctx, 2u) - 0.35) * 0.7;
  (*particle).position = vec4<f32>(cos(angle) * 0.12, 0.2 + sin(angle) * 0.08, 0.0, 1.0);
  ...
  (*particle).size_rotation = vec4<f32>(1.4, 1.4, angle - 1.5707963, 0.0);
  (*particle).lifetime = 0.78 + vfx_random_spawn(ctx, 4u) * 0.35;
}
...
fn vfx_update(ctx: VfxUpdateContext, particle: ptr<function, VfxParticle>) {
  ...
  (*particle).size_rotation.x = mix(1.4, 0.32, life);
