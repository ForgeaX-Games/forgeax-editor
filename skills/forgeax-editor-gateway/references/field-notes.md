# Editor Gateway Field Notes (non-normative)

> Accumulated dogfood edge cases retained for diagnosis. This is not an API contract or a required
> read. Start with `gateway.mjs --help`, `list`, and `help <operation-kind>`; when runtime discovery
> disagrees with this file, the runtime descriptor wins and this note should be repaired.
...
|:--|:--|:--|
| 1. Transport | `gateway.mjs --health` | Reports `pageConnected: true` |
| 2. Capability | `gateway.mjs list` or `gateway.listOps()` | Required operation kind is present |
...
`gateway.mjs list/dispatch`, snippet eval, and `--health` use the standalone relay `:15296`
to reach the connected in-process Editor realm.
If a custom `FORGEAX_BRIDGE_PORT` is set, use the same value for the standalone dev stack and CLI.
The health response also reports `evalTimeoutMs`; this is the relay wait budget,
not an operation completion signal.

> [!CAUTION]
> `gateway.mjs` receives one shell argument. If the snippet contains string literals,
...
## How to drive the Gateway

The Gateway itself is always realm-local. The choice below is only who owns the browser page's
lifecycle; it does not select a different Gateway implementation.

| Page lifecycle | Invocation | Carrier / driver | Use when |
|:--|:--|:--|:--|
| **Attach existing Editor page** (default) | `gateway.mjs list/dispatch/"snippet"` | Loopback DEV relay → page's `__forgeaxEval` | Operate on and inspect the user's actual Editor realm |
| **Create a fresh page** | `gateway-fresh-page.mjs` | Playwright launches Chromium, navigates, then reaches that page's Gateway | CI, cold-load isolation, save→reopen proof |

> [!TIP]
> **Decision rule:** attach with `gateway.mjs` when the current page is evidence. Create a page with
> `gateway-fresh-page.mjs` only when a clean load or reopen is the evidence.

### Quick start — attached page

```bash
# Structured operations against the attached Editor realm.
node skills/forgeax-editor-gateway/scripts/gateway.mjs list
node skills/forgeax-editor-gateway/scripts/gateway.mjs dispatch play --input '{"dirtyPolicy":"last-saved"}'
node skills/forgeax-editor-gateway/scripts/gateway.mjs dispatch stop

# Standalone DEV eval relay.
bun run dev:standalone
node skills/forgeax-editor-gateway/scripts/gateway.mjs --health
node skills/forgeax-editor-gateway/scripts/gateway.mjs "gateway.listOps().length"
node skills/forgeax-editor-gateway/scripts/gateway.mjs --file snippet.js
node skills/forgeax-editor-gateway/scripts/gateway.mjs --file capture.js --timeout 120000
```

### Quick start — fresh page (CI / reopen proof)

```bash
# prereq: bun run test:e2e:install (once) + a running editor
node skills/forgeax-editor-gateway/scripts/gateway-fresh-page.mjs "gateway.listOps().length"
node skills/forgeax-editor-gateway/scripts/gateway-fresh-page.mjs --file snippet.js --settle 0
```
...
> [!IMPORTANT]
> **Driving a continuous op across separate `gateway.mjs` / `gateway-fresh-page.mjs` calls
...
> Also: the mount is **async** and **each `gateway-fresh-page.mjs` call is a fresh page load
...
`page.evaluate` in Playwright, or — preferred — through an attached page (`gateway.mjs`)
...
> ```
> `skills/forgeax-editor-gateway/scripts/gateway-fresh-page.mjs` does this unwrap automatically.
...
> (`skills/forgeax-editor-gateway/scripts/gateway-fresh-page.mjs` waits `--settle` ms, default 1500). Scene-independent calls
...
```ts
// via gateway.mjs or gateway-fresh-page.mjs:
(async () => {
...
`gateway.mjs` is the attached-realm front door. Structured commands and snippets
share the loopback relay; the attached and fresh-page drivers reuse
`gateway-cli-common.mjs` for strict flags and result output.

### Attached page (preferred — `gateway.mjs`)

`gateway.mjs list/dispatch` translates to the public EditGateway surface. All modes route
through the standalone loopback relay to the page's `__forgeaxEval`; none creates a browser.
...
FORGEAX_BRIDGE_PORT=15305 node skills/forgeax-editor-gateway/scripts/gateway.mjs --health
```
...
### Fresh-page verification (`gateway-fresh-page.mjs`)

`skills/forgeax-editor-gateway/scripts/gateway-fresh-page.mjs` owns a new headless browser page,
waits for `__forgeaxEval`
(+ scene settle), evaluates one snippet, awaits it if async, and prints `{ok,value|error}` JSON. Reuse this
...
> [!NOTE]
> `gateway-fresh-page.mjs` takes `--file/--raw/--url/--timeout/--settle`.
> `gateway.mjs` structured mode takes `--origin/--scope/--input/--idempotency-key`;
> eval mode takes `--file/--timeout/--health`.
...
node skills/forgeax-editor-gateway/scripts/gateway-fresh-page.mjs "gateway.listOps().length"                 # scene-independent
node skills/forgeax-editor-gateway/scripts/gateway-fresh-page.mjs "query({with:['Transform']}).rows.length"  # settles for scene first
node skills/forgeax-editor-gateway/scripts/gateway-fresh-page.mjs --raw "typeof world"                       # unlock scope② then eval
node skills/forgeax-editor-gateway/scripts/gateway-fresh-page.mjs --file snippet.js --settle 0               # snippet from file, no settle
```
...
> [!IMPORTANT]
> **The driver owns a page, not another Gateway.** It spawns a **fresh browser page**, then uses the
> same realm-local Gateway. Its evidence is a clean load: save→reopen round-trips and CI isolation.
> Use `gateway.mjs` when the already-open editor and its current state are the evidence.
