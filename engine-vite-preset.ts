...
//   configs consume.
//
// WHY IT LIVES IN the editor config layer (not a browser package)
//   Vite loads this file in Node while bundling each host's config. The engine
//   plugins resolve through edit-runtime's node_modules/@forgeax symlink graph,
//   so this config-time module anchors all producer lookup and dedupe policy at
//   that one graph while remaining consumable by Play, Edit, Studio, and the
//   standalone host.
//
// WHAT IT RETURNS
...
function normalizeGameFilePath(raw: string, viteRoot: string, dereference = true): string {
  let p = raw.replace(/\\/g, '/');
...
export function resolveGameEngineEntry(
  id: string,
  opts: { readonly packageRoots?: readonly string[] } = {},
): string | null {
...
export interface GameSourceResolutionOptions {
  /** Fixed game directory used by standalone Edit/Play hosts. */
  readonly gameDirAbs: string | null;
  /** Multi-game hosts resolve imports from any host-mounted game slug. */
  readonly multiGame?: boolean;
  /** Dynamic host binding; only the current exact game directory may resolve. */
  readonly gameDirProvider?: () => string | null;
  /** Optional exact game directory used by a static single-game build. */
  readonly staticGameDir?: string;
  /** Additional host package graphs (for example Play's NPC adapter package). */
  readonly packageRoots?: readonly string[];
  /** Extra in-root URL prefixes used by a host-mounted multi-game farm. */
  readonly gamePathPrefixes?: readonly string[];
}

function gameEngineResolve(opts: GameSourceResolutionOptions): PluginOption {
  const {
    gameDirAbs,
    multiGame = gameDirAbs === null,
    gameDirProvider,
    staticGameDir = '',
    packageRoots,
    gamePathPrefixes = [],
...
const SHARED_BASE = resolve(EDITOR_ROOT, 'forgeax-editor-assets');
...
function gamePackRoots(gameDirAbs: string): string[] {
  ...
  const templateRuntimeRoots = isGameDefaultTemplate(gameDirAbs)
    ? [
        resolve(ENGINE_ASSETS_BASE, 'sfx'),
        resolve(ENGINE_ASSETS_BASE, 'collectathon-audio'),
        resolve(ENGINE_ASSETS_BASE, 'vendor', 'fbx-test'),
        resolve(ENGINE_ASSETS_BASE, 'khronos-gltf-samples', 'BoxTextured'),
        resolve(ENGINE_ASSETS_BASE, 'demo-assets', 'hello-sprite', 'wood-container.jpg.meta.json'),
        resolve(ENGINE_ASSETS_BASE, 'dejavu-fonts', 'DejaVuSansMono.ttf.meta.json'),
        resolve(ENGINE_ASSETS_BASE, 'dejavu-fonts', 'DejaVuSansMono.atlas.png.meta.json'),
        resolve(ENGINE_ASSETS_BASE, 'dejavu-fonts', 'DejaVuSansMono.font.pack.json'),
        resolve(ENGINE_ASSETS_BASE, 'demo-assets', 'hello-sprite-atlas'),
      ].filter(existsSync)
    : [];
  return existsSync(ENGINE_TEMPLATE_UI_BASE)
    ? [ENGINE_TEMPLATE_UI_BASE, ...templateRuntimeRoots, ...roots]
 ...
 * metadata-driven, with the `.wgsl.meta.json` suffix as a conservative fallback
 * for a sidecar that is still being authored or is temporarily malformed.
 */
export function isEditorBuildOnlyPackPath(path: string): boolean {
...
// pluginPack's scoped middleware matches its routes literally (no base
// awareness); under a non-root base the browser requests arrive prefixed with
...
// pluginPack keys its dev URL map by decoded source paths, while browsers send
// percent-encoded URLs for spaces and non-ASCII filenames. Both the Play
// active-game realm and standalone Edit use the same middleware ordering, so
// keep the normalization at the shared engine boundary.
function decodeAssetUrl(): PluginOption {
  return {
...
export function engineRhiDebugPlugins(
  ...
  return environment.FORGEAX_ENGINE_RHI_DEBUG === '1'
    ? [vitePluginRhiDebug() as unknown as PluginOption]
    : [];
}
...
export function engineVitePreset(opts: EngineVitePresetOptions): EngineVitePreset {
 ...
 // passes an abs dir. Both need bare @forgeax/* re-anchored at edit-runtime.
  ...
  if (process.env.FORGEAX_ENGINE_RHI_DEBUG === '1') {
    plugins.push(...engineRhiDebugPlugins());
  }
