...
// per-game/scene doc-key derivation and the (retired) localStorage doc mirror.
//
...
//
// OOS-1 (zero behavior change): docKey / clearDocStorage /
...
//     plan-strategy §7 M2 (storage cluster split) + D-8.
...
export function createStorage(deps: StorageDeps): Storage {
  ...
  return { loadDocFromStorage, clearDocStorage };
}
