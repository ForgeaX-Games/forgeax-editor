...
| `RhiErrorCode` | rhi | 闭集 union（19 成员，勿抄） | 结构化失败码；`switch` 穷尽无 default |

> [!IMPORTANT]
> 14 句柄 / 7 接口 / 9 descriptor 的**完整名单与签名不在此**——见 `packages/rhi/README.md`。`RhiErrorCode` 的 19 个成员是 `packages/rhi/src/errors.ts` 的 SSOT，**勿抄进 skill**。消费者 tsconfig 的 `compilerOptions.types` 必须含 `"@webgpu/types"`。
...
- `RhiErrorCode` 19 成员闭集（**勿抄**，SSOT）：`packages/rhi/src/errors.ts`
