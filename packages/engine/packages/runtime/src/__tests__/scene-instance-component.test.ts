...
//
// Confirms:
//   - `defineComponent('SceneInstance', { source: 'handle<SceneAsset>',
//      mapping: 'array<entity>', state: 'ref<SceneInstanceState>' })` registers
...
//     handle, the payload is held in the World's ManagedRefStore
//
...
describe('SceneInstance component (w15)', () => {
  ...
  it('source field is handle<SceneAsset>', () => {
    const schema = SceneInstance.schema as Record<string, string>;
    expect(schema.source).toBe('handle<SceneAsset>');
  });
  ...
  it('state field is ref<SceneInstanceState>', () => {
    const schema = SceneInstance.schema as Record<string, string>;
    expect(schema.state).toBe('ref<SceneInstanceState>');
  });
