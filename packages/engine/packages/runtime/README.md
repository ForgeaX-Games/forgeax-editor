...
> feat-20260520-2d-sprite-layer-mvp + feat-20260524 OOS-7 main migration + feat-20260527-sprite-nineslice — sprite is a `MaterialAsset` `passes[0].shader === 'forgeax::sprite'` selector, **not** a sibling asset type. The 9-slice extension adds two optional `paramValues` keys (`slices` + `sliceMode`) consumed by the sprite-pipeline vertex shader; fragment shader, sort formula, RenderQueue, and ECS components remain unchanged (charter P4 consistent abstraction).
...
// mesh handle explicitly.
world.spawn(
  { component: Transform, data: { /* ... */ } },
  { component: MeshFilter, data: { assetHandle: HANDLE_NINESLICE_QUAD } },
  { component: MeshRenderer, data: { materials: [matRes.unwrap()] } },
);
```
...
|:--|:--|:--|:--|
| `slices` | `[left, top, right, bottom]` (4 floats) | each in `[0, 1)`; `left + right < region.z` and `top + bottom < region.w` | corner anchor offsets in **region-local UV (0..1)**, NOT pixels. With `region: [0, 0, 1, 1]`, `slices=[0.25, 0.25, 0.25, 0.25]` means each corner anchor occupies 25% of the region's width / height. |
| `sliceMode` | `0 \| 1` (numeric literal) | closed enum | `0` = stretch (centre cell stretches bilinearly with `Transform.scale`); `1` = tile (centre cell repeats via sampler `addressMode='repeat'`). String literals like `'stretch'` / `'tile'` fail `MATERIAL_PARAM_TYPES_V1` validation. |

> [!CAUTION]
> **`slices` order vs other engines** — `forgeax-engine` uses `[left, top, right, bottom]` (LTRB) which mirrors the sprite UBO `slicesAndMode.xyzw` order (plan-strategy D-3). CSS `border-image-slice` uses TRBL (top-right-bottom-left, clockwise from top); Unity `Sprite.border` uses `Vector4(left, bottom, right, top)`. The order is **not** interchangeable; double-check when porting.

### `sliceMode=1` requires `sampler.addressMode='repeat'`

The tile path leans on hardware sampler wrap (`addressMode='repeat'`) rather than wgsl `fract()` (zero-ALU; D-4 plan-strategy decision aligned with Three.js / Bevy). When `sliceMode=1` is paired with a sampler whose `addressModeU` or `addressModeV` is not `'repeat'`, `AssetRegistry.register` does **not** throw (the sampler may be registered later; D-9 plan-strategy); instead the runtime increments `nineslice.tile-needs-repeat-sampler` on `Renderer.metrics`. The visual silently degrades to clamp-stretch — read the metric counter, do not eyeball the screen.
