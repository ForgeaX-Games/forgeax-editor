...
// ── FORGE_JSON constant (will be in paths.ts per w10) ────────────────────────
const FORGE_JSON = 'forge.json';

// ── helpers ─────────────────────────────────────────────────────────────────

function err(code: GameProjectErrorCode, expected: string, hint: string, detail: GameProjectErrorDetail) {
...
/**
 * Load and validate a game project manifest from the abstract reader.
 *
 * ## Return paths (5)
 *
 * | # | Condition | ok | code |
...
export async function loadGameProject(
  ...
  // ── 3. Schema validate (.strict() rejects unknown fields) ─────────────────
  const schemaResult = GameProjectSchema.safeParse(parsed);
  if (!schemaResult.success) {
    const zodErrors = schemaResult.error.issues;

    // 3a: unknown fields (scenes[] etc.) — detect by looking for unrecognized_keys
    const unknownFieldIssues = zodErrors.filter(
      (i) => i.code === 'unrecognized_keys',
    );
    if (unknownFieldIssues.length > 0) {
      const fieldNames = unknownFieldIssues.flatMap(
    ...
    // 3c: GuidString refinement failure → forge-guid-malformed (D-2 translation)
    const guidFieldIssues = zodErrors.filter(
      (i) =>
        Array.isArray(i.path) &&
        i.path.includes('defaultScene'),
    );
    if (guidFieldIssues.length > 0) {
      // The original raw input for defaultScene before parse
      const rawInput =
    ...
    // 3b: missing required fields or wrong types
    return err(
  ...
  const gameProject: GameProject = schemaResult.data;
...
export async function resolveDefaultScene(opts: {
  read: (path: string) => Promise<string>;
  resolveGuid: (guid: string) => Promise<
