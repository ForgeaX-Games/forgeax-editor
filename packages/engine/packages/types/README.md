...
| Inspector error model | `InspectorErrorCode` / `InspectorError` | 6-member closed union + 4-field structural interface; runtime class lives in `@forgeax/engine-console/errors` (`implements InspectorError`); domain packages import the type alias without dragging the runtime class through static deps (feat-20260511-inspector-p0-spike) |
| Inspector Registry contract | `Registry` / `Handler` / `RegisterRootResult` / `RegisterMethodResult` | Plugin-contributor abstraction SSOT; `@forgeax/engine-{ecs,runtime,pack,gltf}` import only the interface (`import type { Registry } from '@forgeax/engine-types'`); the in-process runtime class lives in `@forgeax/engine-console/registry` and `implements Registry`; same-name `register*` duplicate fails fast with `InspectorError(code='console-startup-failed')` (feat-20260516-console-dependency-inversion) |
| Mutating-methods contributor | `Registry.registerMutatingMethods(names: ReadonlySet<string>) -> RegisterRootResult` | Plugin contributor entry -- domain packages call this once at wire-time so the sandbox can reject mutating method invocations on `ReadOnly` proxies; same-instance (`===`) re-registration fails fast with `InspectorError(code='console-startup-failed')`; distinct sets are accumulated into the merged frozen set returned by `lookupMutatingMethods` (feat-20260517-console-ecs-plugin-extraction D-5) |
| Mutating-methods reader | `Registry.lookupMutatingMethods(): ReadonlySet<string>` | Sandbox-side reader -- returns the merged frozen `Set<string>` of every contributor's names; sandbox.ts reads it once per `wrapReadOnly` and caches in closure (research F6: V8 hash-set O(1), no per-trap cost); identity stable until a new `registerMutatingMethods` call lands (feat-20260517 D-5) |
| Inspector wiring injectors | `WireDefaultInspectorsInjectors` | Function-injection contract for `wireDefaultInspectors`; host passes `registerEcsInspector` + `registerRuntimeInspector` into console so the console package itself never value-imports `@forgeax/engine-{ecs,runtime}` (feat-20260516 round 2 -- physical guarantee behind AC-01 / AC-02 strict 4-deny-list) |
