// viewport-entity-read.test.ts — engine Visibility recursive read helpers
//
...
// Visibility resolver — otherwise a hidden parent's children stay clickable in
// the viewport while the renderer already skips them.

import { describe, it, expect } from 'bun:test';
import { World } from '@forgeax/engine-ecs';
import { ChildOf, Name, Transform } from '@forgeax/engine-scene';
import { Visibility, VisibilityStateValue } from '@forgeax/editor-core';
...
describe('isEntEffectivelyHidden (Visibility inheritance)', () => {
  it('entity with its own hidden Visibility intent is hidden', () => {
    const world = new World();
    const e = spawn(world, 'A');
    expect(isEntHidden(world, e)).toBe(false);
    expect(isEntEffectivelyHidden(world, e)).toBe(false);
    world.addComponent(e, { component: Visibility, data: { state: VisibilityStateValue.hidden } });
  ...
  it('child of a hidden parent is effectively hidden without its own marker', () => {
    ...
    world.addComponent(parent, { component: Visibility, data: { state: VisibilityStateValue.hidden } });
  ...
  it('sibling of a hidden entity stays effectively visible', () => {
    ...
    world.addComponent(hidden, { component: Visibility, data: { state: VisibilityStateValue.hidden } });
