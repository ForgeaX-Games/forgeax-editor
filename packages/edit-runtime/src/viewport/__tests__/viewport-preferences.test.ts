...
describe('viewport preferences persistence', () => {
  ...
  it('round-trips navigation settings and complete camera bookmarks', () => {
    ...
    expect(saveViewportPreferences(preferences, storage)).toBe(true);
    expect(storage.getItem(VIEWPORT_PREFERENCES_STORAGE_KEY)).toContain('"v":1');

    const restored = loadViewportPreferences(storage);
  ...
  it('fails closed for corrupt JSON and unavailable storage', () => {
    ...
    expect(loadViewportPreferences(storage)).toEqual(defaultViewportPreferences());
    expect(saveViewportPreferences(defaultViewportPreferences(), undefined)).toBe(false);
  });
});
