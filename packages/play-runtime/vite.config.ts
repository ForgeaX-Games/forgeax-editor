...
import { targetProfileImporter } from '../engine/templates/game-default/assets/plugins/target-profile-importer';
import {
...
// Per-game pack-index plugin: both dev and production project the catalog
// owned by the one Pack producer. Production must never rescan source sidecars
// here: pluginPack has already finalized/imported the union catalog and owns
// the emitted Pack v2 URLs.
export function forgeaxPerGamePackIndex(pack: ForgeaXPackPlugin) {
  ...
  return {
    ...
    configureServer(server: { middlewares: { use(fn: Function): unknown } }) {
      server.middlewares.use(async (req: { url?: string }, res: { statusCode: number; setHeader(k: string, v: string): void; end(data: string): void }, next: () => void) => {
        ...
        try {
          const catalog = await pack.catalogForRoots(roots);
          res.setHeader('Content-Type', 'application/json');
          res.statusCode = 200;
    ...
    async generateBundle(this: { emitFile(opts: { type: string; fileName: string; source: string }): void }, _opts: unknown, _bundle: unknown) {
      // The Pack producer (registered before us) has already finalized the
      // union catalog. Ask it to project that exact production catalog by the
      // scoped GUID membership; do not rediscover source rows or cook again.
      for (const slug of gameSlugs()) {
        const roots = perGamePackRoots(slug);
        if (roots.length === 0) continue;
        try {
          const catalog = await pack.catalogForRoots(roots, { target: 'build' });
...
const playPackPlugin = pluginPack({
  ...
  // Editor state is retained; catalog and Content Browser update incrementally.
  refresh: () => {},
});
...
export default defineConfig({
  ...
  plugins: [
    ...
    playPackPlugin as never,
    forgeaxPerGamePackIndex(playPackPlugin) as never,
