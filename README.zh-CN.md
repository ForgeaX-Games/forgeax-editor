...
> [!NOTE]
> **「clone 即跑」是有闸门的，不是口号。** CI（[`.github/workflows/ci.yml`](./.github/workflows/ci.yml)）
> 每次 push/PR 都重新证明：全新 `clone → bun install → bun scripts/selfcheck-standalone-b2.mjs`
> 能达到 **自启动等级 B2** —— standalone editor 在**无 studio server** 下读**并写**
> 一个游戏，做法是复用 `@forgeax/platform-io` 作为它的游戏后端（见
> [`standalone/game-backend.ts`](./standalone/game-backend.ts)）。自己跑一下：
> `bun run selfcheck:b2`。

> [!IMPORTANT]
> **完整 standalone 栈**（`bun fx setup` → `bun fx start --game <dir>`）还会构建
> engine 的两个 gitignored wasm 产物（零二进制不变量——编译产物永不入库，始终重建）：
> - **wgpu-wasm** —— 需 Rust + [`wasm-pack`](https://rustwasm.github.io/wasm-pack/)。
> - **fbx-wasm**（浏览器 FBX 导入）—— 需 **Emscripten `emcc`**
>   （`brew install emscripten`，或 [emsdk](https://emscripten.org/docs/getting_started/downloads.html)）。
>
> `bun fx setup` 会构建两者；缺工具链则报错并给出确切安装命令。裸 `bun install`
> **不**构建它们——那条路径只覆盖上面的轻量 dev/B2 流程。缺 `emcc` 仅禁用浏览器
> FBX 导入（glTF 不受影响）。
...
> （见 [`scripts/dev-standalone.ts`](./scripts/dev-standalone.ts)）已替你设好。
...
`FORGEAX_ENGINE_PORT` 可覆盖端口（默认 `15173`）。注意：`bun fx start --play`
开发栈 CLI 会把 play-runtime 跑在 **`:15273`**（它设置 `FORGEAX_ENGINE_PORT=15273`），
以免与占用 `:15173` 的 studio 大仓栈冲突或误杀之。详见下方端口表说明。
...
| `15173` | play-runtime（Play 模式，**原生**路径） | `bun -F @forgeax/editor-play-runtime dev`（vite 默认） |
| `15273` | play-runtime（Play 模式，**`bun fx` 栈**） | `bun fx start --play`（见说明） |
| `18920` | studio-embed host | 完整 studio 栈（studio 仓） |
| `18900` | forgeax-server | 完整 studio 栈（studio 仓） |

> [!NOTE]
> **为什么有两个 play-runtime 端口。** studio 大仓栈（`forgeax-studio scripts/run.ts`）
> 会把_本仓_的 play-runtime 跑在 `:15173`（它的 `PORT_ENGINE` 默认值，依赖
> `vite.config.ts` 的默认端口）。因此 editor 自己的 `bun fx` 开发栈把 play-runtime
> 固定到 `:15273`，并把 `15173` 移出其基于端口的清理集——editor 栈启停时绝不会
> SIGTERM 掉 studio 的 engine，两套栈可并存。原生 `bun -F …play-runtime dev` 路径
> 保留 `15173` 默认值（studio 依赖它）。由 `scripts/lint-fx-no-studio-port.mjs` 守护。
...
| 端口 `15290` / `15280` / `15273` 被占用 | 另一个 vite 实例未停止 | `bun fx stop`、`bash stop.sh`（studio 仓）或手动 `kill` 对应 PID |
| studio 的 `:15173` play-runtime 反复被杀 / studio 浏览器刷满 `:15173 ERR_CONNECTION_REFUSED` | **旧版** editor 的 `bun fx` 清理逻辑把 `:15173` 纳入管辖并误杀了 studio 的 engine | 已修复：editor `bun fx` 栈现在把 play-runtime 跑在 `:15273`，不再碰 `:15173`——把 editor submodule 更新过此修复即可 |
